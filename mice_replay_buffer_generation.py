...
start_path = "H:\\.shortcut-targets-by-id\\1pwsS5t5w38pVgz13-pt77fN4b5YxGlhq\\MacIver Lab  Habitat\\Data\\PEEK_CLEAN"
...
def get_step_information(episode: cw.Episode,
    ...
    def get_observation(prey_step: typing.Optional[cw.Step],
        ...
        prey_direction = cwgame.direction(src=prev_prey_location,
                                          dst=prey_location)
        goal_distance = cwgame.distance(src=prey_location,
                                        dst=goal_location)
        ...
        if predator_step:
            ...
            is_visible = visibility.line_of_sight(src=sp.Point(prey_location), dst=sp.Point(predator_location))

            if is_visible:
                predator_distance = cwgame.distance(src=predator_location,
                                                    dst=prey_location)
        ...
        puff_cooled_down = puff_cool_down_time - last_puff_time_diff
        if puff_cooled_down < 0:
            puff_cooled_down = 0
    ...
    pre_observation = get_observation(prey_step=prey_step,
                                      predator_step=predator_step)
    while prey_step_index < len(prey_trajectory):
        ...
        post_observation = get_observation(prey_step=prey_step,
                                           predator_step=predator_step)
...
for experiment_file_path in experiment_files(start_path):
    ...
    visibility = cwgame.Visibility(arena=loader.arena,
                                   occlusions=loader.occlusions)
    replay_buffer = []
    for episode_number, episode in enumerate(experiment.episodes):
        for pre_observation, action, post_observation in get_step_information(episode=episode,
